...
type key = string;
datatype 'a tree = Leaf | Tree of 'a tree * key * 'a * 'a tree * bool;

val empty = Leaf;

fun insert (key, v, Leaf) = Tree(Leaf, key, v, Leaf, false)
  | insert (key, value, Tree(l, k, v, r, isR)) =
    if key < k
    then Tree(insert(key, value, l), k, v, r, isR)
    else if key > k
    then Tree(l, k, v, insert(key, value, r, isR))
    else Tree(l, key, value, r, isR);
